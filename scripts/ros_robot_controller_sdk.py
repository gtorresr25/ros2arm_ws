...
class PacketFunction(enum.IntEnum):
    ...
    PACKET_FUNC_CONVEYOR = 12  # 传送带的功能号
    PACKET_FUNC_STEPPER_MOTOR = 13 # 步进电机功能号
    PACKET_FUNC_NONE = 14

# 步进电机状态定义 
class StepperState(enum.IntEnum):
    STEPPER_STATE_UNINITIALIZED      = 0 # 未初始化
    STEPPER_STATE_START_INIT         = 1 # 已接收指令，开始初始化流程
    STEPPER_STATE_WAITING_REPOSITION = 2 # 正在等待硬件自动归位完成
    STEPPER_STATE_SETTING_SUBDIVISION= 3 # 正在设置微步细分
    STEPPER_STATE_INITIALIZED        = 4 # 初始化成功，可以随时接收移动指令
    STEPPER_STATE_INIT_FAILED        = 5 # 初始化失败

class StepperInitError(enum.IntEnum):
    """定义了初始化过程中可能发生的具体错误"""
    INIT_OK                         = 0 # 无错误
    INIT_ERR_I2C_UNSPECIFIED        = 1 # 未定义的I2C错误
    INIT_ERR_WRITE_REPOSITION_FAILED= 2 # I2C写入“自动归位”指令失败
    INIT_ERR_READ_REPOSITION_FAILED = 3 # I2C读取“归位状态”失败或归位超时
    INIT_ERR_WRITE_SUBDIVISION_FAILED = 4 # I2C写入“微步细分”设置失败
    INIT_ERR_MOVE_COMMAND_FAILED    = 5 # 在发送移动指令时发生I2C通信失败

STEPPER_ERROR_MESSAGES = {
    StepperInitError.INIT_OK: "无错误。",
    StepperInitError.INIT_ERR_WRITE_REPOSITION_FAILED: "I2C写入“自动归位”指令失败。",
    StepperInitError.INIT_ERR_READ_REPOSITION_FAILED: "I2C读取“归位状态”失败或归位超时。",
    StepperInitError.INIT_ERR_WRITE_SUBDIVISION_FAILED: "I2C写入“微步细分”设置失败。",
    StepperInitError.INIT_ERR_MOVE_COMMAND_FAILED: "在发送移动指令时发生I2C通信失败。",
    StepperInitError.INIT_ERR_I2C_UNSPECIFIED: "发生未定义的I2C错误。"
}
class StepperSubdivision(enum.IntEnum):
    SUBDIVISION_NONE = 0x00  # 1 细分
    SUBDIVISION_2    = 0x01  # 1/2 细分
    SUBDIVISION_4    = 0x02  # 1/4 细分
    SUBDIVISION_8    = 0x03  # 1/8 细分
    SUBDIVISION_16   = 0x07  # 1/16 细分
...
class Board:
    ...
    def __init__(self, device="/dev/ttyUSB0", baudrate=1000000, timeout=5):
        ...
        self.stepper_queue = queue.Queue(maxsize=5)
        ...
        self.parsers = {
            ...
            PacketFunction.PACKET_FUNC_STEPPER_MOTOR: self.packet_report_stepper,
        }

        time.sleep(0.5)
        threading.Thread(target=self.recv_task, daemon=True).start()
  # 步进电机的数据
    def packet_report_stepper(self, data):
        try:
            self.stepper_queue.put_nowait(data)
        except queue.Full:
            pass
    ...
    def set_conveyor_speed(self, speed):
        ...
        :param speed: 速度值，范围是 0-255。0 代表停止。
        """
        speed = int(max(0, min(255, speed)))
        sub_cmd = 0x01
        data_payload = struct.pack("<BB", sub_cmd, speed)
        self.buf_write(PacketFunction.PACKET_FUNC_CONVEYOR, data_payload)
        
    def set_stepper_motor_position(self, steps):
        """
        控制滑轨移动到指定位置

        :param steps: 目标位置的步数, 是一个整数 (例如 5200, -4000 等)
        """

        data_payload = struct.pack('<i', int(steps))
        
        self.buf_write(PacketFunction.PACKET_FUNC_STEPPER_MOTOR, data_payload)
    
    def set_stepper_motor_subdivision(self, subdivision):
        """
        设置步进电机的微步细分值。

        :param subdivision: 细分值。推荐使用 StepperSubdivision 枚举, 例如 StepperSubdivision.SUBDIVISION_8。
                            也可以直接使用整数值：
                            0x00: 1 细分
                            0x01: 1/2 细分
                            0x02: 1/4 细分 (默认)
        ...
        """
        # 设置细分的子命令是 0x04
        STEPPER_SUB_CMD_SET_SUBDIVISION = 0x04
        # 检查参数类型
        subdiv_val = subdivision
        if isinstance(subdivision, StepperSubdivision):
            subdiv_val = subdivision.value
        data_payload = bytes([STEPPER_SUB_CMD_SET_SUBDIVISION, subdiv_val])
        
        self.buf_write(PacketFunction.PACKET_FUNC_STEPPER_MOTOR, data_payload)
        print(f"设置步进电机细分为: {hex(subdiv_val)}")
    ...
    def set_stepper_motor_position(self, steps):
            """
            安全地移动步进电机。此函数会自动处理初始化流程和错误报告。
            这是推荐使用的、唯一的步进电机控制接口。

            :param steps: 目标移动的步数 (整数, 例如 5000, -2000)。
            :return: True 表示指令已成功发送或初始化已成功启动，False 表示发生严重错误。
            """
            STEPPER_SUB_CMD_MOVE = 0x01       
            STEPPER_SUB_CMD_GET_EXT_INFO = 0x03 
            
            print(f"\n===== 开始安全移动程序: 目标 {steps} 步 =====")

            # 1. 查询电机当前状态
            print("--> [第1步/共3步] 查询电机当前状态...")
            
            # 清空队列，防止收到旧的回复
            while not self.stepper_queue.empty():
                self.stepper_queue.get()
            
            # 发送“获取详细状态”指令
            self.buf_write(PacketFunction.PACKET_FUNC_STEPPER_MOTOR, bytes([STEPPER_SUB_CMD_GET_EXT_INFO]))
            
            try:
                # 等待STM32回复，超时1秒
                response = self.stepper_queue.get(block=True, timeout=1.0)
                if not response or response[0] != STEPPER_SUB_CMD_GET_EXT_INFO:
                    print("❌ 错误：查询电机状态的回复无效。")
                    return False
                
                # 从回复中解析出当前状态和错误码
                current_state = StepperState(response[1])
                last_error = StepperInitError(response[2])
                print(f"<-- [回复] 电机当前状态为: {current_state.name}")

            except queue.Empty:
                print("❌ 错误：查询电机状态无响应。请检查接线和固件。")
                return False

            # 2. 根据状态决定下一步操作
            payload_move = struct.pack('<bi', STEPPER_SUB_CMD_MOVE, int(steps))
            
            if current_state == StepperState.STEPPER_STATE_INITIALIZED:
                print("--> [第2步/共3步] 电机已初始化，直接发送移动指令...")
                self.buf_write(PacketFunction.PACKET_FUNC_STEPPER_MOTOR, payload_move)
                print("✅ 指令已发送。安全移动程序结束。")
                return True
            else:
                if current_state == StepperState.STEPPER_STATE_INIT_FAILED:
                    print("--> [提示] 电机上次初始化失败，将尝试重新初始化。")
                    error_message = STEPPER_ERROR_MESSAGES.get(last_error, "未知错误")
                    print(f"    └── 上次失败原因: {last_error.name} ({error_message})")

                print("--> [第2步/共3步] 电机未初始化，发送移动指令以启动初始化流程...")
                self.buf_write(PacketFunction.PACKET_FUNC_STEPPER_MOTOR, payload_move)

                # 3. 轮询等待初始化完成
                print("--> [第3步/共3步] 等待初始化完成 (最长等待30秒)...")
                timeout = 30 # 秒
                ...
                while time.time() - start_time < timeout:
                    ...
                    try:
                        poll_response = self.stepper_queue.get(block=True, timeout=0.4)
                        if poll_response and poll_response[0] == STEPPER_SUB_CMD_GET_EXT_INFO:
                            poll_state = StepperState(poll_response[1])
                            print(f"    - [轮询中] 最新状态: {poll_state.name}")
                            
                            if poll_state == StepperState.STEPPER_STATE_INITIALIZED:
                                print("✅ 初始化成功！电机现在可以自由移动。安全移动程序结束。")
                                return True
                            
                            if poll_state == StepperState.STEPPER_STATE_INIT_FAILED:
                                poll_error = StepperInitError(poll_response[2])
                                error_message = STEPPER_ERROR_MESSAGES.get(poll_error, "未知错误")
                                print(f"❌ 错误：电机初始化失败！")
                                print(f"    └── 根本原因: {poll_error.name} ({error_message})")
                                print("===== 安全移动程序结束 =====")
                                return False
                    except queue.Empty:
                        print("    - [轮询中] 等待回复...")
                ...
                print("⏰ 超时：在15秒内未能完成初始化。请检查电机驱动或归位开关。")
                print("===== 安全移动程序结束 =====")
                return False
...
if __name__ == "__main__":
    ...
    # board.set_stepper_motor_subdivision(StepperSubdivision.SUBDIVISION_4)
    # board.set_stepper_motor_position(5000) 
    #board.set_buzzer(1900, 0.05, 0.01, 2)
    #board.set_stepper_motor_subdivision(StepperSubdivision.SUBDIVISION_4)
    #time.sleep(0.02)
    #board.set_stepper_motor_position(5000) 
    #time.sleep(3) 
    # board.set_stepper_motor_subdivision(StepperSubdivision.SUBDIVISION_8)
    # board.set_stepper_motor_position(-3000) 
    #time.sleep(3) 
